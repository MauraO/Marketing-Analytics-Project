...
The assumptions here were that the quantity of distinct products within a certain category lead to popularity of this category and cheaper products have more substitutes. To verify our aforementioned guess, we came to the question of how many products are contained in each category. Therefore, we plotted the quantity of distinct products on the vertical axis. The output showed that category 8 stored contained the most products, followed by category 5 and 1. Now we saw why categories 1, 5 and 8 were so popular!
...
**Visualization 8. Average Expenditure VS Product Category 1**  
...
According to the result, we found that marital status is not significant in this model. In other word, marital status is not a determinent of log(Purchase). Moreover, the R-square had no change, meaning that marital status can not explain the variance in dependent variable. Therefore, we can draw the conclusion that the marital status is not an omitted variable. 

Secondly, we added "stay_in_current_years" as the control variable and re-performed the regression:
```{r}
lm_multiple_3=lm(log(Purchase)~Gender+Age+City_Category+Stay_In_Current_City_Years,data=demo)
summary(lm_multiple_3)
```
According to the result, we found that stay in current city years is not significant in this model. In other words, stay in current city years is not a determinant of log(Purchase). Moreover, the R-square value had no change, meaning that stay does not explain the variance in the dependent variable. Therefore, we could say that marital status is not an omitted variable. 
