...
Generally, unmarried customers spent more on Black Friday and this held true for most occupations. Specifically, occupations 17, 1, 7, 0 and 4 spent the most money, over $2 trillion, out of which occupation 4 purchased the most. Interestingly, for occupation 10, married customers hardly spent any money on Black Friday.  
...
```
Here we wanted to determine if there were similar purchasing patterns in different age ranges. A boxplot is the most appropriate visualization because age is discrete and purchase is continuous. We improved the plot by setting the theme as sky. The boxplot can answer the question if was there a difference in median purchase among the 7 groups. Customers aged from 18-55 had similar median purchases except for group 36-35, which had the highest median purchase and range. Customers who were under 17 or over 55, however seemed to spend a little bit less.
...
```
The assumptions here were that the quantity of distinct products within a certain category lead to popularity of this category and cheaper products have more substitutes. To verify our guess, we were curious as to how many products were in each category. Therefore, we plotted the quantity of distinct products on the vertical axis. The output showed that category 8 stored contained the most products, followed by category 5 and 1. Now we saw why categories 1, 5 and 8 were so popular!
...
**Test 2. Do unmarried customers spend more money on Black Friday than married customers?**
```{r, echo=TRUE, message=FALSE, warning=FALSE}
#Independent t-test
...
```
According to the result, we found that stay in current city years was not significant in this model. In other words, stay in current city years was not a determinant of log(Purchase). Moreover, the R-square value was unchanged, indicating that stay did not explain the variance in the dependent variable. Therefore, we could say that marital status was not an omitted variable. 

Next, we added occupation as the control variable and re-performed the original regression:z
```{r}
...
```
The above code shows a visualization of the sum of squared distance for k in range 1-15. The 'elbow' of the visualization shows the optimal number of clusters, which in this dataset was 4. The 4 segments differed in age, occupation, city category, marital status and stay in current city years.
...
* Cluster 3: Customers in this group spent the second most on Black Friday. Most customers were male (72.74%) and married (80.85%). They were also young, i.e., two thirds were aged 26-35. Roughly half of total customers were from city A and most (72) had been in their current city for over 3 years.  
...
```
After using the prediction of mean log(Purchase) for each customer, we found that out of the top 10 highest average actual purchases from customers: 70% were male, 60% were from city A and 40% from city B, and 40% from the age range of 46-50. 

For the predicted top 10 highest average actual purchases from customers: 100% were male, 90% were from city B, and 80% were from the age range of 26-45.
...
After performing predictive analyis on the customer segments, we found that men from city B in the age range of 26-45 were predicted to be the most profitable customer. It is important to note, however, that while this segment is crucial, it is important to not direct all marketing dollars on it. This is because the actual data showed more of a mix: some women, some older, most from city A. Therefore, we suggest that the company try a riskier (but  possibly much more rewarding) marketing campaign on the up-and-coming cluster 4, and continue more traditional, "tried but true" campaigns for the other clusters.
